...
//}
	...
	$reponse=$bdd->prepare("SELECT p.CIP,p.DESIGNATION,p.DCI,p.QTE_STOCK, p.PRIX_PRODUIT, p.QTE_STOCK * p.PRIX_PRODUIT AS VALEUR FROM produit p ORDER BY p.designation");
	$reponse->execute();
}
	...
	<table id="table" margin-top style="font-size: 8px; width: all;" align="center">
	       ...
	       <thead>
				<tr>
					...
					<th style="width: unset;">Montant</th>
				</tr>
		  </thead>
		   ...
		   <tbody>	
                  <?php while ($donnees = $reponse->fetch()){ ?>
                    <tr class="odd gradeX">
                        ...
                        <td style="width: unset;"><?php echo $donnees['PRIX_PRODUIT']; ?></td>
                        <td style="width: unset;"><?php echo $donnees['QTE_STOCK']; ?></td>
                        <td style="width: unset;"><?php echo number_format($donnees['VALEUR'], 2, ',', '.'); ?></td>
                    </tr>
                <?php
            	 $total+=$donnees['VALEUR'];
                 } ?>
		  </tbody>
          <tfoot>
            <tr>
                <th style="text-align:center" colspan="5" class="col_num">TOTAL</th>
                <th style="text-align:center" colspan="1" class="col_num"><?php echo number_format($total, 2, ',', '.'); ?></th>
            </tr>
        </tfoot>	
	</table>
